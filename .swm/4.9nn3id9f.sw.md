...
pinpoints, and source-specific aliases. Full-case extension retains trailing
reporters, bracket aliases, and pinpoints in that order, including a pinpoint
after the declared alias. Normalization standardizes whitespace, reporter
casing, party formatting, and numeric citation components without changing the
...
Before commit it flushes pending rows, records a citation comparison, and rejects a rebuilt short form
that lacks direct anchor provenance or a nonempty-to-empty result. This is separate from generic and
short-form target resolution and later metric recomputation.
