...
matches report-owned assessment paragraph numbers only to unique canonical
`CaseChunk.paragraph_start` values, preserves chunk IDs, hashes, and offsets,
and reports exact, ambiguous, missing-assessment, and unmatched states. Case
citations and statute references remain separate evidence arrays keyed by
their existing `chunk_id`; no cross-chunk inference is permitted. The
2026-09-24 scan produced 11,657 exact matches, 798 ambiguous mappings, and
969 missing assessment rows. It found no statute references attached to the
cohort paragraph chunks and no citation rows attached to those paragraph
chunks, so any additive SQL bridge must first reconcile chunk-set ownership.
