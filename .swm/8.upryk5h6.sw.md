...
The bounded `scripts/dry_run_paragraph_evidence_bridge.py` command extends
that inspection to the 300-case core cohort without creating linkage rows. It
matches report-owned assessment paragraph numbers to canonical
`CaseChunk.paragraph_start` values, preserves chunk IDs, hashes, and offsets,
and reports exact, ambiguous, missing-assessment, and unmatched states. Since
current citation rows are stored on `section` chunks, the report-only bridge
also translates section-local citation offsets into paragraph-local offsets
when an exact paragraph-text span contains the full citation. The 2026-09-25
artifact retained 11,657 exact paragraph matches, 798 ambiguous mappings, 969
missing assessment rows, and 6,260 translated citation links across 3,241
paragraph records. Original citation IDs and source chunk IDs remain intact;
unmapped citations are not assigned by guesswork. Statute references remain a
separate evidence layer, and no citation-treatment classification is performed.

The active paragraph assessment retention boundary is
`paragraph_level_300_run`; older five-case, 20-case, single-case, and
request-only paragraph pilots were retired on 2026-09-24. The prepared
Discussion Unit material remains separately scoped to `core_300_run`.

The active Core Cases workflow loads up to 100 ordinary results from the
`discussion_unit_core_300` allowlist. Once activated, its second assessment
search calls `/analytics/search/cohort-assessments` and searches the complete
300-case report-only bridge using transparent token overlap across topic, role,
explanation, and paragraph text. Each hit retains its case and paragraph
identity, match reasons, and available citation IDs. This is a bounded
lexical proof of concept: it does not use embeddings, modify canonical rows,
or infer how a citation was treated.
Assessment hits expose a `Compare topic/role` action for deterministic
cross-case review. Comparison groups retain the paragraph-level identity tuple
`(case_id, paragraph, chunk_id, citation_id)` so stored citations remain tied
to the assessment paragraph that produced them. The UI opens the originating
source case; it does not infer a cited authority's legal treatment.
The active generated bridge preserves translated paragraph-local citation
offsets and canonical `target_case_id` values when available. Core 300 citation
chips open the resolved target authority; unresolved links fall back to the
source case and remain explicitly unresolved.
...
The treatment-teacher contract now supports an argument-level citation event,
not only a polarity label. A validated label may carry exact source spans for
the actor who raised or used the authority, the reason it was raised, the
argument supported, the argument addressed by the Court, the Court's response,
and the conclusion for that argument. Each field may also be explicitly
`not_stated`, `not_applicable`, or `ambiguous`; unsupported inferences are
rejected. The argument conclusion is kept separate from the decision's overall
disposition, and distillation preserves this context without publishing it to
runtime behavior.

The Mason proving-ground execution path is now bounded for external review.
`scripts/build_mason_argument_citation_fixture.py` retains the owning citation
chunk and authoritative offsets while sending only the citation paragraph plus
bounded neighboring paragraphs. `scripts/prepare_treatment_teacher_batch.py`
creates the no-network request, and
`scripts/run_treatment_teacher_batch.py` defaults to one-event calls,
checkpoints raw responses, enforces the paid budget, and validates every
returned span. The original three-event run cost `$0.0315425` and accepted 12
labels, while the isolated 97-event rerun cost `$0.0366286` and accepted 31
labels with only one malformed response. Two isolated labels populated
argument-level context fields. This demonstrates a cheap, repeatable review
workflow but not semantic readiness; runtime publication remains disabled.
The separate case-level dossier experiment sends the deterministic Mason record
and identified citation windows to `gpt-4.1` in one report-only call. The first
call cost `$0.413526` but selected only three citation treatments. The corrected
compact contract cost `$0.432228` and returned valid JSON covering all 97/97
identified citation IDs, classified as 85 supportive and 12 neutral, in
`mason_case_intelligence_result_exhaustive_compact.json`. This richer output
remains subject to human review and is not runtime-published. Review found a
strong supportive-label skew, repetitive rationales, and a cross-reference
defect: argument `A2` names citation ID `7658423`, which is absent from the
citation-treatment map. It must remain provisional and cannot populate
canonical treatment rows.
The complete review surface is
`scripts/build_mason_citation_review_ledger.py`. It preserves one row for
every deterministic Mason citation event and distinguishes an accepted label,
a rejected label, a malformed batch response, and an omitted label. This
prevents response-format failures from disappearing events from human review;
the ledger remains provisional and does not publish treatment behavior.
...
## Bounded Discussion Unit Cohort

For a larger report-only labeling sample, use
`scripts/select_discussion_unit_cohort.py`. It unions an explicit priority CSV
with up to 2,500 other cases ranked by inbound citation count. Priority cases
can be older than 2005; ranked additions require `Case.date >= 2005-01-01` and
use deterministic tie-breaking by decision date and local case ID. The JSON
artifact records the inclusion reason, count source, rank, and projected cost.
Selection makes no external calls and writes no database rows. The default
curated core-case CSV is only a reproducible input convention; a final flagged
case list should be passed explicitly when it differs from that artifact.

The current feature-testing artifacts are generated by
`scripts/build_discussion_unit_priority_lists.py`:
`data/eval/llm_discussion_units_pilot/discussion_unit_core_300.csv` is the
smallest layer, and
`data/eval/llm_discussion_units_pilot/discussion_unit_priority_2500.csv`
contains all core IDs plus 2,200 ranked additions. The expanded totals are 500
SCC, 750 FCA, and 1,250 FC. Core rows are labeled `core_300`; additions are
labeled `priority_2500` and are the only rows subject to the 2005 cutoff.

## Resumable External Review

The model-assisted labeling path is coordinated by
`scripts/run_discussion_units_cohort.py` and checkpointed by
`scripts/discussion_units_ledger.py`. A manifest maps each case to its
deterministic input report, request JSON, and Markdown output. The runner never
resends a terminal case by default; `--retry-failed` is required for failed
rows. Network errors, malformed responses, and replay failures are recorded in
the ledger, while raw malformed responses and per-case Markdown remain available
for visual review. This is still report-only: no canonical citations, statutes,
tags, outcomes, offsets, or database rows are changed. See
`docs/NEXT_STEPS.md` for the 300-case-first execution sequence.

The core-300 preparation checkpoint completed all 300 deterministic reports and
request payloads. The approved first-10 API validation completed 9 cases and
retained one raw response for failed case `677`; no later cases were attempted.
The remaining 290 calls require explicit approval. Database-owned persistence is
deferred to a separate design and implementation task so the report-only
artifacts remain isolated from canonical data.
...
The active Citation Intelligence Overview now adds a compact case fingerprint
without creating another top-level tab. It combines existing overview metrics,
up to eight stored inbound citation rows, and up to six shared-authority
neighbors. Each inbound row opens the active inline reader; shared-authority
counts are derived navigation signals only. The UI keeps stored citation
evidence separate from derived clusters and does not infer legal treatment,
similarity, or controlling status.

The Overview now keeps the selected authority context visible and exposes
explicit paths to Timeline, Neighborhood, and stored Evidence. Its enrichment
requests are independent: a missing fingerprint, cluster, or signal response
does not replace the authority summary with a stale placeholder. Loading,
empty, and request-failure states remain visible in the Citation Intelligence
content area, and neighborhood activation is bound once so a tab click does
not duplicate the request. The owning renderer and focused contract are
`backend/pages/data_explorer.py` and `tests/test_feature_tabs.py`.

Timeline years now provide the next traceability step: selecting a year opens
the existing citation evidence table with that year's stored rows, and a clear
control returns to the unfiltered table. The interaction remains inside the
Citation Intelligence subviews and does not create a new navigation surface.

The Overview also surfaces up to four ranked authority signals from the
existing citation-map route. Occurrence, section spread, and surprise are
shown as navigation signals only; they do not establish legal importance or
citation treatment. Each row preserves the linked case identity and opens the
active reader.

The Citation Intelligence Neighborhood subview now provides a bounded direct
relationship view around the selected case. It labels whether a related case
cites the focus case or is cited by it, preserves occurrence counts, and opens
the related decision in the active reader. The graph remains a navigation aid,
not a legal-similarity or treatment classifier.
