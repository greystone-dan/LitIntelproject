...
| `case_processing.py` | Ordered 5-stage deterministic case processing orchestrator | `case_id` -> Populated derived DB tables | Stages: (1) full_case -> (2) heading_chunks -> (3) metadata -> (4) case_citations -> (5) statutes. |
...
3. **Enrichment & Extraction**:
   ...
   - `resolve_citation_targets.py` / `resolve_short_citation_targets.py`: Local target resolution, powered by a reusable local case index. Exact citation variants include bracketed, bare, and parenthesized reporter years; formal/name rows can use unique canonical-title recovery, and duplicate titles can use an exact cited decision year only when one target remains. Short/name rows can use preserved composite anchors and repeated globally resolved aliases only when collision and self-case gates pass. The case-citation path remains separate from statute reference extraction and does not pre-merge the two layers.
   - Full reported citations such as `[2004] 3 F.C.R. 323` are eligible same-document anchors for later short forms; full authority text and exact source offsets are preserved. Historical rows created before this extraction rule require a separately bounded backfill if provenance must be repaired.
   - The 2026-09-07 anchor inventory found `78,372` `case_short` rows missing anchor fields out of `1,408,402`; V2 treats these as provenance defects, not valid unanchored citations. Task 068 covers that backfill. A separate second backfill, task 069, covers the `383,608` unresolved rows with existing anchors; it found `9,130` pure right-edge extension candidates, `14,235` start-correction candidates, and `360,243` rows with no local extension evidence. The bounded follow-ups must repair anchor fields only and must not alter target resolution; no writer has run.
   - The 2026-09-07 whole-inventory checkpoint inspected 747,051 unresolved rows and added 2,343 title-year links, reaching 2,152,332 total rows, 1,407,624 linked targets, and 744,708 unresolved rows. Ambiguous year sets, anonymized, truncated, single-name, and locally absent authorities remain unresolved for source or adjudication work.
