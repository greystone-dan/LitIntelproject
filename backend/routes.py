...
.advanced-actions{margin-top:12px}.advanced-search{margin-top:12px;padding-top:12px;border-top:1px solid var(--border)}.advanced-search[hidden]{display:none}.advanced-search .search-form{margin-top:10px}.check-field{display:flex;align-items:center;gap:8px;min-height:42px;color:var(--muted);font-size:12px;font-weight:600}.check-field input{width:16px;height:16px;accent-color:var(--blue)}
...
document.querySelectorAll('[data-tab]').forEach(tab=>tab.addEventListener('click',()=>{const active=tab.dataset.tab;const panels={about:'aboutPanel',search:'searchPanel','citation-intelligence':'citationIntelligencePanel',judge:'judgePanel','judge-profile':'judgeProfilePanel',explorer:'explorerPanel'};const selected=panels[active]?active:'search';Object.entries(panels).forEach(([key,id])=>{const panel=document.getElementById(id);if(panel)panel.hidden=key!==selected});if(selected==='judge')document.getElementById('judgePanel').hidden=false;if(selected==='judge-profile')loadJudgeProfiles();}));document.getElementById('citationIntelligenceSearch').onsubmit=searchCitationCases;document.getElementById('judgeProfileSearch').onsubmit=searchJudgeProfiles;const initialTab=new URLSearchParams(location.search).get('tab');if(initialTab==='judge-profile'){document.getElementById('searchPanel').hidden=true;document.getElementById('judgeProfilePanel').hidden=false;loadJudgeProfiles();}else if(initialTab==='judge-outcomes'){document.getElementById('searchPanel').hidden=true;document.getElementById('judgePanel').hidden=false;loadJudge();}
